    ...
    def add(self):
        ...
        with ui.tab_panels(curve_tabs, value=default_tab, animated=False).classes("w-full").style("min-height: 300px"):
            if configs[n].get("cpu_temp_type", "None") != "None":
                with ui.tab_panel(cpu_curve_tab).classes("w-full"):
                    with ui.row().classes("w-full items-center justify-around"):
                        self._add_single(name=n, sensor="cpu", label="Low Power", temp_default=30)
                        self._add_single(name=n, sensor="cpu", label="Balanced", temp_default=40)
                        self._add_single(name=n, sensor="cpu", label="Performance", temp_default=50)
                        self._add_single(name=n, sensor="cpu", label="High Power", temp_default=60)
                        self._add_single(name=n, sensor="cpu", label="Max Power", temp_default=70)
            if configs[n].get("drive_temp_type", "None") != "None":
                with ui.tab_panel(drives_curve_tab).classes("w-full"):
                    with ui.row().classes("w-full items-center justify-around"):
                        self._add_single(name=n, sensor="drive", label="Low Power", temp_default=30)
                        self._add_single(name=n, sensor="drive", label="Balanced", temp_default=40)
                        self._add_single(name=n, sensor="drive", label="Performance", temp_default=50)
                        self._add_single(name=n, sensor="drive", label="High Power", temp_default=60)
                        self._add_single(name=n, sensor="drive", label="Max Power", temp_default=70)
            if configs[n].get("gpu_temp_type", "None") != "None":
                with ui.tab_panel(gpu_curve_tab).classes("w-full"):
                    with ui.row().classes("w-full items-center justify-around"):
                        self._add_single(name=n, sensor="gpu", label="Low Power", temp_default=30)
                        self._add_single(name=n, sensor="gpu", label="Balanced", temp_default=40)
                        self._add_single(name=n, sensor="gpu", label="Performance", temp_default=50)
                        self._add_single(name=n, sensor="gpu", label="High Power", temp_default=60)
                        self._add_single(name=n, sensor="gpu", label="Max Power", temp_default=70)
        ...
        if row is not None:
            with row:
                with ui.expansion(name, icon="bar_chart").classes("w-full") as expansion:
                    ctrl_type = configs[name].get("speed_ctrl_type", "None")
